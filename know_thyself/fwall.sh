...
iptables -A INPUT -p tcp -m multiport --dports 22,443 -s 127.0.0.1,192.168.1.0/24 -m conntrack --ctstate NEW -j ACCEPT
iptables -P INPUT DROP

#output - has to be opened for ccs client
iptables -A OUTPUT -m conntrack --ctstate RELATED,ESTABLISHED -j ACCEPT
#port connections for NEEDED dependencies
iptables -A OUTPUT -p tcp -m multiport --dports 3306 -d 127.0.0.1,192.168.1.0/24 -m conntrack --ctstate NEW -j ACCEPT
